...
namespace Heinermann.ShadowMinion
{
  public class ShadowAi : GameStateMachine<ShadowAi, ShadowAi.Instance>
  {
    ...
    public override void InitializeStates(out BaseState default_state)
    {
      ...
      dead.ToggleStateMachine((Instance smi) => new FallWhenDeadMonitor.Instance(smi.master))
        .ToggleBrain("dead").Enter("RefreshUserMenu", (Instance smi) => smi.RefreshUserMenu())
        .Enter("DropStorage", (Instance smi) => smi.GetComponent<Storage>().DropAll(false, false, default(Vector3), true));
    }
  }
}
